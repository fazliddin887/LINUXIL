...
class InlineManager:
    ...
    async def _create_bot(self) -> None:
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            m = await conv.send_message(f"☁️ LINUXIL Userbot of {self._name}")
            r = await conv.get_response()
        ...
        if not query:
            ...
            await inline_query.answer(
                [
                    InlineQueryResultArticle(
                        id=rand(20),
                        title="Показать доступные встроенные команды ",
                        description=f"Ваши {len(_help.splitlines())} встроенные команды",
                        input_message_content=InputTextMessageContent(
                            f"<b>🍭️ Встроенные инлайн команды:</b>\n\n{_help}",
                            "HTML",
