...
def main():  # noqa: C901
        ...
        if web:
            ...
            if not arguments.heroku_web_internal:
                ...
                if platform.system() == "Linux" and not os.path.exists(
                    ...
                    print(f"Посетите... http://localhost:{port}")
                else:
                    ipaddress = get("https://api.ipify.org").text
                    print(
                        f"Посетите... http://{ipaddress}:{port} or http://localhost:{port}"
                    )
            ...
            if not web.running.is_set():
                ...
                if not arguments.heroku_web_internal:
                    ...
                    if platform.system() == "Linux" and not os.path.exists(
                        ...
                        ipaddress = get("https://api.ipify.org").text
                        print(
                            f"Посетите сайт http://{ipaddress}:{port} or http://localhost:{port}"
                        )
    ...
    if arguments.heroku:
        if isinstance(arguments.heroku, str):
            ...
            key = input(
                "Вставьте свой Heroku API token (from https://dashboard.heroku.com/account): "
            ).strip()

        app = heroku.publish(clients, key, api_token)
        print(
            "Успешно установлено на Heroku ! Введите команду .help в своем приложении Telegram"
        )  # noqa: T001
        if web:
